package uk.metricSpaceFramework.searchStructures;
...
public abstract class SearchIndex<T> {
    ...
    protected T chooseMaxiMinPivot(Set<T> pivots, List<T> dat) {
        /*
         * we have a set of existing pivot data, and a list from which to
    ...
    public static double projectionDistance(double ab, double aq, double bq) {
        if (ab == 0) {
            throw new RuntimeException("coincident reference data");
        }
        if (aq == 0) {
            return 0;
        } else if (bq == 0) {
    ...
    /**
     * for three data p1, p2 and s; gives the perpendicular distance from p1p2
